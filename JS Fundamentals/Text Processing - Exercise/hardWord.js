function hardWord(text) {
    let [letter, wordsArr] = text;

    let words = letter.split(' ');
    let blankSlots = words.filter(word => word.includes('_'));

    for (slot of blankSlots) {
        if (!slot.endsWith('_')) {
            slot = slot.slice(0, slot.length - 1)
        }

        let fillingWords = wordsArr.find(word => word.length == slot.length);
        letter = letter.replace(slot, fillingWords)
    }
    console.log(letter);
}
